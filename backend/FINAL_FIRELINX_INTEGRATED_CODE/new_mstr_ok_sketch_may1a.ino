...
const char* mqtt_server = "259353f6c5704a35aeb3dff107a0ab04.s1.eu.hivemq.cloud"; //"YOUR_MQTT_BROKER_IP_OR_HOSTNAME"; // <<-- REPLACE
const int mqtt_port = 8883; //1883;                                  // Default MQTT port
const char* mqtt_topic_subscribe = "staferb/web_alerts"; //"firelinx/master/alerts";  // <<-- REPLACE with your topic
const char* mqtt_client_id = "esp32_FireLinx_Receiver";             // Unique client ID (change if needed)
// Add user/pass if your broker requires authentication
const char* mqtt_user = "Staferb";
const char* mqtt_password = "EspWebDash@32";

// WiFiClient espClient;
// PubSubClient mqttClient(espClient);
WiFiClientSecure espClientSecure;          // Use the secure client object
PubSubClient mqttClient(espClientSecure); // Pass the secure client to 
...
void initMQTT() {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        // >>> ADD THIS LINE TO BYPASS CERTIFICATE CHECK (INSECURE!) <<<
        espClientSecure.setInsecure();
        // >>> END ADD <<<

        mqttClient.setServer(mqtt_server, mqtt_port);
        mqttClient.setCallback(mqttCallback);
        // Optional: Modify log message
        Serial.println("MQTT Client Configured (INSECURE MODE)."); // Indicate insecure mode
    } else {
...
bool reconnectMQTT() {
    ...
    // Attempt to connect using Client ID, Username, and Password
    // >>> MODIFY THIS LINE to include user/pass <<<
...
// ========================================================================
// == MORE ROBUST parseAndEnqueueMqttMessage FUNCTION ==
// == (Includes whitespace trimming and more tolerant handling) ==
// ========================================================================

// Helper function to trim leading/trailing whitespace from a C-string IN PLACE
void trimWhitespace(char *str) {
    ...
    *(end + 1) = '\0';
    ...
    // Shift string if leading spaces were removed
    if (str != start) {
        memmove(str, start, strlen(start) + 1); // Shift content, including the new null terminator
    }
}
...
void parseAndEnqueueMqttMessage(String jsonInput) {
    ...
    Serial.println("MQTT: Attempting to parse JSON...");

    DeserializationError error = deserializeJson(doc, jsonInput);

    if (error) {
        Serial.print(F("MQTT JSON deserializeJson() FAILED!: ")); Serial.println(error.f_str());
        Serial.print("Input was: "); Serial.println(jsonInput);
        return;
    }
    Serial.println("MQTT: JSON Deserialization SUCCESS.");

    if (!doc.is<JsonObject>()) {
        Serial.println(F("MQTT JSON Error: Expected a single JSON object '{...}' at the root."));
        return;
    }

    JsonObject commandObject = doc.as<JsonObject>();
    const char* command = commandObject["command"];

    if (!command || strcmp(command, "fire_alert") != 0) {
        Serial.println("MQTT Warning: Message ignored, command is not 'fire_alert' or is missing.");
        return;
    }

    if (!commandObject["payload"].is<JsonObject>()) {
         Serial.println("MQTT Error: Received 'fire_alert', but 'payload' is not a JSON object.");
         return;
    }

    Serial.println("MQTT: Received valid 'fire_alert' command with object payload.");
    JsonObject payloadObject = commandObject["payload"].as<JsonObject>();
    struct_message tempData = {0}; // Initialize struct
    const char* tempStr = nullptr;
    bool criticalError = false; // Flag critical errors preventing enqueue

    // --- Extract, Trim, and Validate ---

    // 1. Fire Type (Critical)
    tempStr = payloadObject["fireType"];
    if (tempStr) {
        char typeBuf[2] = {0}; // Buffer for single char + null
        strncpy(typeBuf, tempStr, 1); // Get only first char
        trimWhitespace(typeBuf); // Trim (handles if " A " was sent)
        if (strlen(typeBuf) == 1 && (typeBuf[0] >= 'A' && typeBuf[0] <= 'D')) {
            tempData.fireType = typeBuf[0];
        } else {
            Serial.printf("MQTT Parse Error: Invalid fireType ('%s'). Message discarded.\n", tempStr);
            tempData.fireType = '?'; criticalError = true;
        }
    } else {
        Serial.println("MQTT Parse Error: Missing 'fireType'. Message discarded.");
        tempData.fireType = '?'; criticalError = true;
    }

    // 2. Fire Intensity (Critical)
    tempStr = payloadObject["fireIntensity"];
     if (tempStr) {
        char intensityBuf[2] = {0};
        strncpy(intensityBuf, tempStr, 1); // Get only first char
        trimWhitespace(intensityBuf);
        if (strlen(intensityBuf) == 1 && (intensityBuf[0] >= '1' && intensityBuf[0] <= '4')) {
            tempData.fireIntensity = intensityBuf[0];
        } else {
            Serial.printf("MQTT Parse Error: Invalid fireIntensity ('%s'). Message discarded.\n", tempStr);
            tempData.fireIntensity = '?'; criticalError = true;
        }
    } else {
        Serial.println("MQTT Parse Error: Missing 'fireIntensity'. Message discarded.");
        tempData.fireIntensity = '?'; criticalError = true;
    }

    // 3. Verified (Non-critical default)
    if (payloadObject["verified"].is<bool>()) {
        tempData.verified = payloadObject["verified"].as<bool>();
    } else {
        // Try parsing string "true" (case-insensitive)
        tempStr = payloadObject["verified"];
        if (tempStr && strcasecmp(tempStr, "true") == 0) {
             tempData.verified = true;
             Serial.println("MQTT Parse Info: Parsed 'verified' from string \"true\".");
        } else {
            Serial.println("MQTT Parse Warning: 'verified' missing or invalid type/string. Defaulting to false.");
            tempData.verified = false;
        }
    }

    // 4. User Name (Non-critical, trim, truncate)
    tempStr = payloadObject["user"];
    if (tempStr) {
        strncpy(tempData.user, tempStr, sizeof(tempData.user) - 1);
        tempData.user[sizeof(tempData.user) - 1] = '\0'; // Ensure termination before trim
        trimWhitespace(tempData.user); // Trim in place
        if (strlen(tempStr) >= sizeof(tempData.user)) { // Check original length for truncation
             Serial.println("MQTT Parse Warning: 'user' was truncated.");
        }
    } else {
         Serial.println("MQTT Parse Warning: 'user' missing. Leaving empty.");
         tempData.user[0] = '\0';
    }

    // 5. User ID (Non-critical, trim, truncate)
    tempStr = payloadObject["userID"];
     if (tempStr) {
        strncpy(tempData.userID, tempStr, sizeof(tempData.userID) - 1);
        tempData.userID[sizeof(tempData.userID) - 1] = '\0';
        trimWhitespace(tempData.userID);
        if (strlen(tempStr) >= sizeof(tempData.userID)) {
             Serial.printf("MQTT Parse Warning: 'userID' ('%s') was truncated.\n", tempStr);
        }
    } else {
        Serial.println("MQTT Parse Warning: 'userID' missing. Leaving empty.");
        tempData.userID[0] = '\0';
    }

    // 6. Station ID (Critical - must be non-empty after trim)
    tempStr = payloadObject["stnID"];
    if (tempStr) {
        strncpy(tempData.stnID, tempStr, sizeof(tempData.stnID) - 1);
        tempData.stnID[sizeof(tempData.stnID) - 1] = '\0';
        trimWhitespace(tempData.stnID); // Trim first
        if (strlen(tempData.stnID) == 0) { // Check if empty after trim
             Serial.printf("MQTT Parse Error: 'stnID' ('%s') is missing or empty after trim. Message discarded.\n", tempStr);
             criticalError = true;
        } else if (strlen(tempStr) >= sizeof(tempData.stnID)) { // Check original length
             Serial.println("MQTT Parse Warning: 'stnID' was truncated.");
        }
    } else {
        Serial.println("MQTT Parse Error: 'stnID' missing. Message discarded.");
        tempData.stnID[0] = '\0';
        criticalError = true;
    }

    // 7. Latitude (Non-critical, trim, truncate, default)
    tempStr = payloadObject["latitude"];
    if (tempStr) {
        strncpy(tempData.latitude, tempStr, sizeof(tempData.latitude) - 1);
        tempData.latitude[sizeof(tempData.latitude) - 1] = '\0';
        trimWhitespace(tempData.latitude);
         if (strlen(tempStr) >= sizeof(tempData.latitude)) {
             Serial.println("MQTT Parse Warning: 'latitude' was truncated.");
        }
        if (strlen(tempData.latitude) == 0) { // If trimming made it empty, set N/A
             Serial.println("MQTT Parse Warning: 'latitude' became empty after trim. Setting to 'N/A'.");
             strncpy(tempData.latitude, "N/A", sizeof(tempData.latitude));
        }
    } else {
        Serial.println("MQTT Parse Warning: 'latitude' missing. Setting to 'N/A'.");
        strncpy(tempData.latitude, "N/A", sizeof(tempData.latitude));
    }

    // 8. Longitude (Non-critical, trim, truncate, default)
    tempStr = payloadObject["longitude"];
    if (tempStr) {
        strncpy(tempData.longitude, tempStr, sizeof(tempData.longitude) - 1);
        tempData.longitude[sizeof(tempData.longitude) - 1] = '\0';
        trimWhitespace(tempData.longitude);
         if (strlen(tempStr) >= sizeof(tempData.longitude)) {
             Serial.println("MQTT Parse Warning: 'longitude' was truncated.");
        }
         if (strlen(tempData.longitude) == 0) {
             Serial.println("MQTT Parse Warning: 'longitude' became empty after trim. Setting to 'N/A'.");
             strncpy(tempData.longitude, "N/A", sizeof(tempData.longitude));
        }
    } else {
         Serial.println("MQTT Parse Warning: 'longitude' missing. Setting to 'N/A'.");
         strncpy(tempData.longitude, "N/A", sizeof(tempData.longitude));
    }

    // 9. Date (Non-critical, trim, truncate)
    tempStr = payloadObject["date"];
     if (tempStr) {
        strncpy(tempData.date, tempStr, sizeof(tempData.date) - 1);
        tempData.date[sizeof(tempData.date) - 1] = '\0';
        trimWhitespace(tempData.date);
        if (strlen(tempStr) >= sizeof(tempData.date)) {
             Serial.printf("MQTT Parse Warning: 'date' ('%s') was truncated.\n", tempStr);
        }
    } else {
        Serial.println("MQTT Parse Warning: 'date' missing. Leaving empty.");
        tempData.date[0] = '\0';
    }

    // 10. Time (Non-critical, trim, truncate)
    tempStr = payloadObject["time"];
    if (tempStr) {
        strncpy(tempData.time, tempStr, sizeof(tempData.time) - 1);
        tempData.time[sizeof(tempData.time) - 1] = '\0';
        trimWhitespace(tempData.time);
         if (strlen(tempStr) >= sizeof(tempData.time)) {
             Serial.println("MQTT Parse Warning: 'time' was truncated.");
        }
    } else {
        Serial.println("MQTT Parse Warning: 'time' missing. Leaving empty.");
        tempData.time[0] = '\0';
    }

    // --- Final Check and Enqueue ---
    Serial.printf("MQTT DEBUG: Final criticalError flag = %s\n", criticalError ? "true" : "false");

    if (!criticalError) {
        Serial.println("MQTT Parse successful (cleaned/validated). Enqueueing.");
        portENTER_CRITICAL(&queueMux);
        fireAlertQueue.push(tempData); // Enqueue the cleaned struct
        portEXIT_CRITICAL(&queueMux);
        Serial.println("   Added to queue (MQTT - Object Payload).");
    } else {
        Serial.println("MQTT Parse Error: Critical field(s) invalid or missing after cleaning. Message DISCARDED.");
        // Log the partially cleaned/defaulted data that was discarded for debugging
        Serial.println("--- Discarded MQTT Data ---");
        logAlert(tempData);
        Serial.println("---------------------------");
    }
}
...
float convertCoordinate(const char* ddm) {
    ...
    int scanResult = sscanf(ddm, "%d°%f'%c", &degrees, &minutes, &direction); //int scanResult = sscanf(ddm, "%d*%f'%c", &degrees, &minutes, &direction);
    if (scanResult < 2) {
